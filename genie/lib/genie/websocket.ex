defmodule Genie.Websocket do
  ...
  def start_link(state) do
    ...
    WebSockex.start_link("#{url}?token=#{token}", __MODULE__, state, name: __MODULE__, async: true, handle_initial_conn_failure: true)
  end
  ...
  defp handle_message(%{"event" => "phx_reply", "payload" => %{"response" => %{}, "status" => "ok"}}) do
